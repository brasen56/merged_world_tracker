...
By default tests run in Node with **no DOM**. A handful of UI tests need a real-enough one; those files opt in with a first-line `/** @vitest-environment jsdom */` pragma and run in [jsdom](https://github.com/jsdom/jsdom), a dev dependency (`test/modal_accessibility.test.js`, `test/modal_inert_guards.test.js`, `test/kt_view_modal_singleton.test.js`, `test/kt_chat_change_sweep.test.js`). If your test only needs a few DOM-ish objects, prefer stubbing them locally the way `modal_interactions.test.js` builds its parsed-DOM fake.
...
- **Modal / DOM-adjacent rendering** — `modal_interactions.test.js` drives the shared `core/modal.js` lifecycle (Escape rule, × / backdrop pointers, the onClose veto on the × / backdrop / Escape / `hideModal` routes, handler cleanup) against a minimal parsed-DOM fake; `backup_ui.test.js` pins the backup presenters. The half that needs a real DOM runs in **jsdom** (see the environment note above): `modal_accessibility.test.js` (dialog semantics, focus-on-open and focus restore, the focus trap, the `hideModal` veto), `modal_inert_guards.test.js` (the managed-inert backdrop: fail-safes, foreign-dialog refusal including its user-visible toast, native `<dialog>` ranking), `kt_view_modal_singleton.test.js` (the `#kt-view-modal` singleton race through the real render wiring), and `kt_chat_change_sweep.test.js` (the Knowledge chat-change close path restoring inertness). The modals inside each module's `render.js` no longer bypass the shared lifecycle — the four Knowledge modals run it through `decorateModalShell`, and the jsdom suites above pin them. *Still out:* anything visual/CSS.
