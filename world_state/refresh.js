...
function validateFullRefreshOutput(text) {
    const repaired = normalizeGeneratedDocument(text, { plotSeedContract: usesBuiltInPlotSeedContract() });
 ...
 * holding only a placeholder ("None.") becomes a removal, and so does a Plot
 * Seeds replacement whose every seed was unsalvageable.
 *
 * Repairing the generated bytes here is what breaks the delta ratchet: the
 * patched document is the next delta's "Previous World State", so a malformed
 * seed that reaches storage becomes the exemplar the model imitates. Saved
 * sections the patch does not touch keep their own style.
 */
function repairDeltaOperations(ops) {
    const plotSeedContract = usesBuiltInPlotSeedContract();
    return ops.map(operation => {
        ...
        const repaired = normalizeGeneratedDocument(withHeader, { plotSeedContract });
