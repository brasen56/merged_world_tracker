...
**Date:** 2026-09-14  
**Source:** Story Planner module review, 2026-09-13  
...
The default full-plan prompt should carry opaque arc and beat identifiers in
...
1. a valid existing arc/beat identifier emitted from the built-in prompt;
2. an unambiguous exact normalized title/text match for compatibility;
3. a new identifier.
...
If the model omits every identifier, the existing title fallback keeps custom and
less compliant models usable. Planted/skipped beats from the stored arc are still
preserved exactly; only the pending suffix is eligible for replacement.
...
most recently closed titles and concise reasons. It communicates:
...
The projection is bounded by whole records and characters/tokens. Pinned closed
records rank first, then recently closed records. Omitted records stay stored and
...
Each phase should land independently. Phases 0–2 are the reliability foundation;
later phases can be scheduled according to user demand.
...
1. Add the Phase 0 regression fixtures.
2. Ship the v2 migration and stable beat objects without changing the visible UI.
3. Replace the regeneration merge and add durable closed memory.
4. Build the full beat editor on the v2 data model.
5. Add Park/Resume, Focus, Archive presentation, and injection-mode cleanup.
6. Add targeted arc proposals and Generate setup beats.
7. Add manual, evidence-backed progress checking.
8. Add the compact story palette.
9. Add safe, opt-in Knowledge character grounding after the shared cast/scene seam
   is available.
10. Complete documentation, manual QA, measurements, and the optional-automation
    decision records.
